...
min_total_area = 50  # m² (nach Multiplikation mit Stockwerken)
buildings = buildings[buildings["total_area_m2"] >= min_total_area]

# ---------------------------
# ENERGIEBERECHNUNG (HEIZUNG)
# ---------------------------
EUI = {
    "residential": 180,
    "commercial": 250,
...
# ---------------------------
co2e_factors = {
    "Gas": 0.203, # conversion factors for UK, Natural Gas
    "Electric": 0.177, # conversion factors for UK 
...
co2_factor_network = 0.08 # Annahme für CO2-Faktor des Wärmenetzes (kann je nach Quelle variieren)

def calc_co2_after(row):
    current_factor = co2e_factors[row["heating_type"]]
    
    # Nur anschließen, wenn das Netz besser ist
...
print("FERTIG: Karten + Tabelle erstellt!")
...
# Beispiel einzelnes Residential

# Annahmen UK-Mittelwerte
house_area_m2 = 100       # Annahme einer zufälligen gerundeten Wohnfläche, ausgesucht aus der erstellten Karte
levels = 2                 # Stockwerke
total_area = house_area_m2 * levels

EUI_residential = 180       # kWh/m²/Jahr für Heizung
heating_share = 0.65        # Anteil für Heizung
COP_heatpump = 4.3

# Gas-Heizung vorher
heating_energy_yr = total_area * EUI_residential * heating_share  # kWh/Jahr
co2_factor_gas = 0.203
cost_gas_per_kWh = 0.10 # Annahme

co2_before = heating_energy_yr * co2_factor_gas / 365  # kg/Tag
cost_before_day = heating_energy_yr * cost_gas_per_kWh / 365  # £/Tag

# Wärmenetz / Wärmepumpe nachher
...
print("\n==============================")
print("EINZELNES RESIDENTIAL-Haus")
print("==============================")
print(f"Fläche: {total_area} m² ({levels} Stockwerke)")
print(f"Heizenergie pro Jahr: {heating_energy_yr:,.0f} kWh")
print(f"CO2 vorher: {co2_before*365:.1f} kg/Jahr")
print(f"CO2 nachher: {co2_after*365:.1f} kg/Jahr")
print(f"CO2-Ersparnis: {reduction_pct:.0f}%")
print(f"Kosten vorher (Gas): £{cost_before_month:.2f}/Monat")
print(f"Kosten nachher (Netz + WP): £{cost_after_month:.2f}/Monat")
print(f"Monatliche Einsparung: £{savings_month:.2f}")
print(f"Prozentuale Einsparung: {100 * savings_month / cost_before_month:.1f}%")
...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # zwingt Matplotlib, kein GUI zu benutzen
import matplotlib.pyplot as plt

# ---------------------------
# Parameter
initial_investment = 2500  # £ für Wärmetauscher + Anschluss
years = 20
discount_rate = 0.05
annual_savings = 76.48 * 12  # £/Jahr

# Cashflows
cashflows = np.array([-initial_investment] + [annual_savings]*years)

# Diskontierte Cashflows
discounted_cashflows = cashflows / (1 + discount_rate) ** np.arange(0, years+1)

# Kumulativer DCF
cumulative_dcf = np.cumsum(discounted_cashflows)

# ---------------------------
# Plot erstellen und speichern
plt.figure(figsize=(10,6))
plt.plot(range(0, years+1), cumulative_dcf, marker='o', color='green')
plt.axhline(0, color='black', linestyle='--')
plt.title("Discounted Cash Flow (DCF) for Single Household with District Heating")
plt.xlabel("Year")
plt.ylabel("Cumulative DCF (£)")
plt.grid(True)

# Als PNG speichern
plt.savefig("DCF_single_household.png", dpi=300)
plt.close()

# Solarpark, Spitzen decken und Thermalspeicher (1energy, 3 thermalspeicher)

#Current Energy Situation in Oxford: 
# - High reliance on fossil fuels
# -  Growing focus on renewables

# Our proposed heat network system
# Total CO2reduction 43,7% 
# Over 1300 connected buildings

# reduction in residential from gas 80!

# Einzelne Residentialfolie
